...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» DITTO")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🇩𝐈𝐓𝐓Ⓞ 🇧​​​​Ⓞ𝐓__\n» `{mp} ᴍꜱ`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f" RUKO JARA SABAR KARO...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __DITTO KA NAYA LADKA A GYA...__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f" DITTO KA LADKA HAI YAAR !!")
        else:
            if len(sudousers) > 0:
                newsudo = f"{sudousers} {target}"
            else:
                newsudo = f"{target}"
            await ok.edit(f"» **DITTO KA NAYA LADKA A GYA**: `{target}`\n» `ʀᴇsᴛᴀʀᴛɪɴɢ ʙᴏᴛ...`")
